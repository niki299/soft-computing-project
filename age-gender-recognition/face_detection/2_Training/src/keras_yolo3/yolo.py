...
from age.age_model import predict_age
from gender.gender_model_2 import predict_gender
...
class YOLO(object):
    ...
    def detect_image(self, image, show_stats=True):
        ...
        font_path = os.path.join(os.path.dirname(__file__), "font/Ethiopic_WashRa_SemiBold.ttf")
...
def draw_rectangle(predictions, age_pred, gender_pred, image):
    ...
    # (randrange(222), randrange(222), randrange(222))
    color = (224, 228, 213)
    ...
    for i, prediction in enumerate(predictions):
        ...
        label = "{} {:.2f}, Age {age}, Gender {gender}".format(predicted_class, score, age = age_pred[i], gender=gender_pred[i])
        label_size = draw.textsize(label, font)
...
def detect_webcam(yolo):
    ...
    while vid.isOpened():
        ...
        result = np.asarray(display_image)
        cv2.namedWindow("Result", cv2.WINDOW_NORMAL)
        cv2.imshow("Result", result)
        cv2.waitKey(1)
